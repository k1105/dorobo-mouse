    ...
    this.root.innerHTML = `
      <div class="lobby-panel wide">
        ...
        <div class="controls-help">
          ...
          <p>🐭 ネズミ: WASD or 矢印キーで移動。好きな商品棚の近くで「盗む」ボタンを押し続け、そのまま${CONFIG.stealTimeSec}秒とどまると盗み成功（ボタンを離すと中断）（その間、体が左右に揺れて目立つ！）→ 出口(緑ゲート)から持ち出すと商品の値段分のスコアを獲得。持ち出した後はカメラの死角にリスポーンして何度でも狙える</p>
          <p>🎥 猫: 画面中央のカメラマップでカメラ番号をクリック（or 数字キー）してオンライン⇔オフライン切替（同時${CONFIG.maxViewCams}台まで）。映像内の怪しいネズミをクリックでダウト（1ラウンド各${CONFIG.doubtsPerRound}回まで）。的中したネズミは商品を没収されて退場（以降は店内全体を俯瞰で観戦）、NPCなら空振り。ラウンドは時間切れで終了</p>
          <p>💡 オンラインのカメラは球体が赤く発光し、視野に入っている床が明るくなる（ネズミからも「みられている場所」が分かる）</p>
          <p>🐱 猫: 画面中央のカメラマップにはオンラインのカメラの視野（赤）と死角が表示される（Mキーで表示/非表示）。🐭 ネズミは同じマップが左下に常時表示され、自分の位置が分かる</p>
